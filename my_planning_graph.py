...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set())                         # Defined empty set here                       

        for s_level in self.s_levels[level]:                # Traversing through different S-nodes
            for action in self.all_actions:                 # Going through all actions available
                action_node = PgNode_a(action)              # Getting A-node for current action
                pre_nodes = action_node.prenodes            # Getting Parent as previous S-node for that A-node
                if s_level in pre_nodes:                    # If Parent of A-node matches list of previous S-nodes then doing mapping
                    s_level.children.add(action_node)       # Linking A-node as child to it's S-node
                    action_node.parents.add(s_level)        # Linking S-node as Parent to A-node
                    self.a_levels[level].add(action_node)   # Adding A-node to current level
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set())                     # Defined empty set here 

        for pre_a_node in self.a_levels[level - 1]:     # Traversing through previous action nodes
            poss_s_nodes = pre_a_node.effnodes          # Getting possible S nodes for current level

            for s_node in poss_s_nodes:                 # Traversing to map them into graph
                s_node.parents.add(pre_a_node)          # Linking S-node to previous A-node as parent
                pre_a_node.children.add(s_node)         # Linking previous A-node to S-node as child
                self.s_levels[level].add(s_node)        # Adding S-node to current level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        pairs = [[node_a1.action.effect_add, node_a2.action.effect_rem],           # Identified pairs of action.effect that required to be tested 
            [node_a1.action.effect_rem, node_a2.action.effect_add]]                # Effect of one action must negate effect of another action
        for a,b in pairs:                                                          # Traversing through each pairs
            for i in a:                                                            # Choosing one effect to check with other
                if i in b:                                                         # Checking whether one action.effect present in another list or not
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        pairs = [[node_a1.action.effect_add, node_a2.action.precond_neg],          # Identified pairs of action.effect that required to be tested 
            [node_a1.action.effect_rem, node_a2.action.precond_pos],               # Effect of one action must negate the precondition of the other
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for level,nodeset in enumerate(self.s_levels):   # Going through all S levels
                literals = [s.literal for s in nodeset]      # Collecting all literals at each level
                
                if goal in literals:                         # Checking Goal in that list
                    level_sum = level                        # Returning level value if found
                    break
